...
local function CollectEnemies(targetList, folder)
    for _, entity in ipairs(folder:GetChildren()) do
        ...
        if headPart and IsLiving(entity) and Player:DistanceFromCharacter(headPart.Position) < FastAttack.Range then
...
function FastAttack:ExecuteAttack(targetList)
    if #targetList > 0 then
        for _, targetData in ipairs(targetList) do
            local target, headPart = unpack(targetData)
            RegisterAttackEvent:FireServer(_G.FastAttackDelay or 0.02)
            RegisterHitEvent:FireServer(headPart, { target })
        end
    end
end
...
function FastAttack:FindAndAttackNearest()
    ...
    if FastAttack.TargetMobs then
        CollectEnemies(potentialTargets, EnemyFolder)
    end
    if FastAttack.TargetPlayers then
        CollectEnemies(potentialTargets, CharacterFolder)
    end

    if #potentialTargets > 0 then
        self:ExecuteAttack(potentialTargets)
    else
        task.wait(0.5)
    end
end
...
function FastAttack:HandleBladeHits()
    ...
    if not IsLiving(character) then
...
-- Vòng lặp chính để xử lý FastAttack
task.spawn(function()
    while true do
        ...
        if not success then
            warn("[FastAttack Error]:", err)
        end
        task.wait(_G.FastAttackDelay or 0.125)
    end
end)
